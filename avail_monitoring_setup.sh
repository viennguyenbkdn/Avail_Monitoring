...
if [ $? -eq 0 ] ; then
        ...
        systemctl restart $Avail_systemd; sleep 1;
        echo -e "\nEditing and Restarting current Avail systemd service..."
else
        ...
        if [ $? -eq 0 ]; then
                ...
                sleep 1;
                docker cp ./entrypoint.sh ${Avail_container}:/entrypoint.sh > /dev/null ; sleep 1 ;
                docker restart $Avail_container > /dev/null; sleep 1;
                echo -e "\nEditing and Restarting current Avail containter..."
        fi
fi
